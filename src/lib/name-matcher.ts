...
export async function matchNames(query: string, names: string[]): Promise<string[]> {
  ...
  if (!cache || cache.key !== cacheKey) {
    ...
    for (const n of norms) {
      const out = (await embedder(n, { pooling: 'mean', normalize: true } as any)) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const qOut = (await embedder(qNorm, { pooling: 'mean', normalize: true } as any)) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
  const qVec = Array.from(qOut.data) as number[];
